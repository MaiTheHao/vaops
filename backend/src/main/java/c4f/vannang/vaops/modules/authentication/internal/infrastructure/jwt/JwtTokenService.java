package c4f.vannang.vaops.modules.authentication.internal.infrastructure.jwt;
...
public class JwtTokenService implements TokenProviderStrategy {
    ...
    public String createAccessToken(AccessTokenClaims claims) {
        ...
        return Jwts.builder().id(UUID.randomUUID().toString()).issuer(authProperties.getJwt().getIssuer())
                .claims(claims.toClaimsMap()).issuedAt(new Date(now))
                .expiration(new Date(now + authProperties.getJwt().getAccessExpirationMs())).signWith(accessKey)
                .compact();
    }
    ...
    public String createRefreshToken(RefreshTokenClaims claims) {
        ...
        return Jwts.builder().id(UUID.randomUUID().toString()).issuer(authProperties.getJwt().getIssuer())
                .claims(claims.toClaimsMap()).issuedAt(new Date(now))
                .expiration(new Date(now + authProperties.getJwt().getRefreshExpirationMs())).signWith(refreshKey)
                .compact();
    }
    ...
    public AccessTokenClaims validateAccessToken(String token) {
        try {
            Claims claims = Jwts.parser().verifyWith(accessKey).requireIssuer(authProperties.getJwt().getIssuer())
                    .build().parseSignedClaims(token).getPayload();
            return new AccessTokenClaims(UUID.fromString(claims.get("userId", String.class)), claims.getSubject());
    ...
    public RefreshTokenClaims validateRefreshToken(String token) {
        try {
            Claims claims = Jwts.parser().verifyWith(refreshKey).requireIssuer(authProperties.getJwt().getIssuer())
                    .build().parseSignedClaims(token).getPayload();
